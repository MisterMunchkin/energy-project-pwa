...
export default async function page() {
  ...
  return (
    <section className="py-[50px]">
      <TopNavBar title="National Statistics" showAccount />
      <Suspense fallback={<span>...Loading</span>}>
        <div className="p-2 flex flex-col">
          <Link className="cursor-pointer" href={`${PUBLIC_LEADERBOARD}`}>
            <VscArrowLeft className="text-epp-indigo w-10 h-10" />
          </Link>
          <NationalStatisticsBarChart chartData={chartData} />
        </div>
        <div className="flex flex-col p-4">
          <span className="font-bold text-3xl">
            {nationalStats.totalWHSOfTheNation}
          </span>

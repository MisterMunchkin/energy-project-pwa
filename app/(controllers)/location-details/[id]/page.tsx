...
const LocationDetails = async ({ params }: ServerComponentProps) => {
  ...
  if (!idString)
    throw new Error(
      "passing `id` is required for `LocationDetails`. id: " + idString,
    );

  return (
    <>
      <LocationSummary locationId={idString} />
      <div className="flex flex-col space-y-8 p-4 bg-epp-white scroll-auto rounded-t-2xl">
        <LocationStats locationId={idString} />
        <Appliances locationId={idString} />
      </div>
    </>
  );
};
